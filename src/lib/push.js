...
// (POST /api/notifications/devices). notify.js on the server then delivers pushes
// to every registered token for the user. If the VITE_FIREBASE_* env vars are
// empty, this is a silent no-op so local dev/demo keeps working.
...
export async function registerPush() {
  ...
  if (!firebaseConfig.apiKey || !VAPID_KEY) return;
  ...
  // The SW is served from public/ at the site root.
  ...
  // axios baseURL already includes /api → this hits /api/notifications/devices.
